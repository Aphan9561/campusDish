...
def get_menu_data(location, meal_id, date):
    ...
    response = requests.get(f'https://sc-campusdish-com.translate.goog/api/menu/GetMenus?locationId={location_id}&date={date}&periodId={period_id}')
...
def get_schedule_data(restaurant: str) -> dict:
    ...
    try:
        url = 'https://sc.campusdish.com/LocationsAndMenus/'
        if restaurant == 'GibbesCourtBistro':
            url += 'GibbesCourtBistro'
        else:
            url += restaurant
        schedule = {}
